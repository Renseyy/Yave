...
void YAVE_exec_keys(void){
    ...
    float dz=glm::cos(cam0.rotation[0]) * ndy;
        ...
        if(YAVE_keys[YAVE_KEY_UP].pressed) cam0.position  += glm::vec3(-cam0.v[0]*dx, cam0.v[1]*dy, cam0.v[2]*dz);
        if(YAVE_keys[YAVE_KEY_DOWN].pressed) cam0.position  += glm::vec3(cam0.v[0]*dx, -cam0.v[1]*dy, -cam0.v[2]*dz);
        ...
        if(cam0.rotation[0]>=2*M_PI) cam0.rotation[0]-=2*M_PI;
        if(cam0.rotation[1]>=2*M_PI) cam0.rotation[1]-=2*M_PI;
        if(cam0.rotation[0]<0) cam0.rotation[0]+=2*M_PI;
        if(cam0.rotation[1]<0) cam0.rotation[1]+=2*M_PI;
}
